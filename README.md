...
- StillWorkingOn.js: Page layout and performance can be optimized - but it's better to fill the corresponding pages with actual content.
...
- Add Latinized forms of Persian words and store them externally (or generated by algorithm, or written into the database in advance), with a toggle to determine whether to display them.
...
## 03/02/2025 Update:
- Replace the PUKPortraitUnhappy and create a separate folder to store PUK images.
- (App.js, LessonNavigator.js, Vocabulary.js) Replace the position of Vocabulary and LessonNavigate so that the index of vocabulary is LessonNavigate. The paths in the corresponding .js files have also been corrected. - The 01/02/2025 Update [ (Home.js, NavigationBar.js, home.css) Change the start page of vocabulary function from vocabulary of lesson 1 to the lesson navigator, so it's the same as the exercise function. ] has been rolled back, except for the css modification.
- (index.html) No longer "React App" as the title but the with the full name of "IonianFarsi - آیونیان فارسی". I also optimized the head icon and saved it in the same directory as IonianFarsiLogo64%.jpg but it is not yet implemented.
- (Vocabulary.js) Beautify choose-lesson button: Since it is designed to have a fixed effect, I currently only center the text.
- (App.css, Words.js) Beautify the words page: More margins, bigger headings. There is still more that can be beautified but it will take time.

![03/02/2025 Update gif](./puk-preview/03022025.gif)

![03/02/2025 Update png](./puk-preview/03022025.png)
...
- (App.js) Add a test page for testing. Εννοείται. The directory is named dokimi so that it is not easy to find. Can be set to be accessible to administrators only in the future.
...
- Fix bug: "Failed to fetch ... data" or keep "Loading, you may need to wait up to 50 seconds in first load" if page is inactive for a while; One simple refresh does not solve the problem, but restarting from the home page does. This seems to be solved by adding a redirect link to restart the app on the corresponding page but I'm not sure. Of course, optimizing database connections is the only way to solve the problem once and for all. (03/02/2025 Note: Tests have proven that this method can indeed solve the problem.)
...
- (NavigationBar.js, LessonNavigator.js, exercise.js, app.css) Add text centerer to center the text. (03/02/2025 Note: I didn't know that bootstrap was already imported so I could just use it. Ugh.)
